  ...
  return (
    <div className="space-y-3">
      <div className="flex gap-1.5">
        ...
        <button onClick={handleRandom} className="px-2.5 py-1.5 text-xs bg-warm hover:bg-line text-ink rounded transition-colors">随机</button>
        <button onClick={resetLines} className="px-2.5 py-1.5 text-xs bg-warm hover:bg-line text-soft rounded transition-colors">重置</button>
      </div>

      <div className="space-y-1.5 text-xs">
        <div>
          <div className="flex items-center justify-between mb-1"><label className="text-soft">干支（四柱）</label>
            <button onClick={handleAutoFillSizhu} className="text-[11px] text-matcha-dim hover:text-matcha transition-colors">自动填充</button>
          </div>
          <div className="grid grid-cols-4 gap-1.5">
            <input value={sizhuYear} onChange={(e) => setSizhu('year', e.target.value)} placeholder="年" className="bg-cream border border-line rounded px-2 py-1.5 text-ink text-xs" />
            <input value={sizhuMonth} onChange={(e) => setSizhu('month', e.target.value)} placeholder="月" className="bg-cream border border-line rounded px-2 py-1.5 text-ink text-xs" />
            <input value={sizhuDay} onChange={(e) => setSizhu('day', e.target.value)} placeholder="日" className="bg-cream border border-line rounded px-2 py-1.5 text-ink text-xs" />
            <input value={sizhuHour} onChange={(e) => setSizhu('hour', e.target.value)} placeholder="时" className="bg-cream border border-line rounded px-2 py-1.5 text-ink text-xs" />
          </div>
        </div>
        <div><label className="text-soft block mb-1">空亡（日柱）</label>
          <input value={kongWang} onChange={(e) => setKongWang(e.target.value)} placeholder="如 辰巳" className="w-full bg-cream border border-line rounded px-2 py-1.5 text-ink text-xs" /></div>
      </div>

      <div className="flex items-center gap-1.5 px-1 text-[10px] text-soft">
      ...
      <div className="bg-cream rounded px-2 py-1 border border-line/50">
        <div className="text-[10px] text-matcha-dim mb-1 px-7">本卦{benGuaName ? `：${benGuaName}` : ''}</div>
        {orderedLines.map((line) => (<YaoLineRow key={line.position} line={line} onChange={(field) => updateYaoLine(line.position, field)} />))}
      </div>

      {hasChanging && (
        <div className="bg-cream rounded px-2 py-1 border border-rust/20">
      ...
      <div className="text-xs"><label className="text-soft block mb-1">备注</label>
        <textarea value={beizhu} onChange={(e) => setBeizhu(e.target.value)} placeholder="补充信息，如伏神说明、特殊状态等" className="w-full h-16 bg-cream border border-line rounded px-2 py-1.5 text-ink text-xs resize-none" /></div>
    </div>
  )
}
