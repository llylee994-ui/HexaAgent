  ...
  return (
    <div ref={containerRef} className="relative flex-1">
      <input type="text" value={search} placeholder={placeholder || '搜索卦名...'} onFocus={() => setOpen(true)}
        ...
        className="w-full bg-ink border border-line rounded px-2 py-1.5 text-xs text-cream placeholder-warmgray focus:outline-none focus:border-gold-dim" />
      {open && filtered.length > 0 && (
        <div className="absolute z-10 mt-1 w-full max-h-40 overflow-y-auto bg-ink border border-line rounded shadow-lg">
          {filtered.map((name) => (
            <button key={name} onClick={() => { onSelect(name); setSearch(name); setOpen(false) }}
              className="w-full text-left px-2 py-1.5 text-xs text-cream hover:bg-gold/10 hover:text-gold transition-colors">{name}</button>
          ))}
        </div>
      )}
    </div>
