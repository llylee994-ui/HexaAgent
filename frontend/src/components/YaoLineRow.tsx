...
export default function YaoLineRow({ line, onChange, isAutoMode, isChanged }: Props) {
  ...
  const sel = "bg-ink border border-line rounded px-1 py-0.5 text-[10px] text-cream disabled:opacity-40"

  return (
    <div className="border-b border-line/50">
      <div className="flex items-center gap-1.5 py-1 text-xs">
        <span className="w-7 text-warmgray text-center">{posLabel}</span>

        {isChanged ? (<span className="w-10 text-center text-[10px] text-gold-dim/30">承</span>) :
         isAutoMode ? (<span className="w-10 text-center text-[10px] text-gold-dim">{line.liushen || '-'}</span>) :
         (<select value={line.liushen} onChange={(e) => onChange({ liushen: e.target.value })} className={`${sel} w-11 text-gold-dim`}><option value="">-</option>{LIUSHEN.map(ls => <option key={ls} value={ls}>{ls}</option>)}</select>)}

        {readOnly || isChanged ? (
          <span className="w-12 text-center text-[10px] text-cream">{line.type === 'yang' ? '---' : '- -'}</span>
        ) : (
          <div className="flex rounded overflow-hidden border border-line">
            <button className={`px-1.5 py-0.5 text-[10px] ${line.type === 'yang' ? 'bg-gold-dim text-ink' : 'bg-ink text-warmgray'}`} onClick={() => onChange({ type: 'yang' })}>---</button>
            <button className={`px-1.5 py-0.5 text-[10px] ${line.type === 'yin' ? 'bg-gold-dim text-ink' : 'bg-ink text-warmgray'}`} onClick={() => onChange({ type: 'yin' })}>- -</button>
          </div>
        )}
        ...
        <label className="flex items-center gap-0.5 cursor-pointer">
          ...
          <span className={line.changing ? 'text-rust text-[10px]' : 'text-warmgray text-[10px]'}>动</span>
        </label>
        ...
        {!readOnly && !isChanged && (
          <button onClick={() => setShowFush(!showFush)} className={`ml-auto text-[10px] px-1 rounded transition-colors ${hasFush ? 'text-plum bg-plum/5' : 'text-warmgray hover:text-plum'}`}>{hasFush ? '伏' : '+'}</button>
        )}
      </div>
